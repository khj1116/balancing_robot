...
// SoftwareSerial 핀 설정 (RX, TX 핀을 원하는 핀 번호로 설정)
...
#endif
    mySerial.begin(115200); // 원하는 보드레이트 설정
    Serial.begin(115200);
...
//------------------------------------------------------------
 if (mySerial.available()) {
        ...
        // 입력 데이터 파싱
        char paramType = inputString.charAt(0); // 첫 글자로 매개변수 선택
        double newValue = inputString.substring(1).toDouble(); // 나머지 부분은 값으로 처리
